 ...
 *  Copyright (C) 2000  Lars C. Hassing  lch@ccieurope.com
 *
...
/*****************************************************************************
  Please do not edit this file. Instead contact Lars C. Hassing (lch@cci.dk)
...
000703 lch Release v1.3: added -i and -o options
                                                                             */
...
char                *ProgVer = "mklist  v1.3 20000703  (C) 1999-2000 Lars C. Hassing  lch@ccieurope.com";
...
int                  main(int argc, char **argv)
{
   ...
   SkipTilde = 0;
   SkipMovedto = 1;
   strcpy(PartsDir, "PARTS");
   strcpy(PartsLst, "parts.lst");
   while (--argc > 0)
   {
      arg = *++argv;
      if (arg[0] == '-')
   ...
   if (PartsDir[strlen(PartsDir) - 1] != '\\')
      strcat(PartsDir, "\\");
   Pos = strlen(PartsDir);
   strcat(PartsDir, "*.*");
   for (done = findfirst(PartsDir, &ffb, 0); !done; done = findnext(&ffb))
   {
      strcpy(PartsDir + Pos, ffb.ff_name);
      fp = fopen(PartsDir, "rt");
      if (!fp)
      {
         printf("Cannot open \"%s\".", PartsDir);
         PressAnyKey();
         continue;
      }
