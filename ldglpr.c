...
void write_png(char *filename)
{
  ...
  if (downsample) {
    ...
    for(i = 0; i< 256; i++)
      GAMMA[i] = (unsigned int) (4095.0 * pow((i/255.0), gamma));
    for(i = 0; i< 4096; i++)
  ...
  for (i = height-1; i >= 0; i--)
  {
	...
	*p++ = a | b;
	...
	a &= 0xff000000;
	r  = GAMMA[(p0[j+0] & 0xff0000) >> 16] << 0;
	r += GAMMA[(p0[j+1] & 0xff0000) >> 16] << 1;
	r += GAMMA[(p0[j+2] & 0xff0000) >> 16] << 0;
	r += GAMMA[(p1[j+0] & 0xff0000) >> 16] << 1;
	r += GAMMA[(p1[j+1] & 0xff0000) >> 16] << 2;
	r += GAMMA[(p1[j+2] & 0xff0000) >> 16] << 1;
	r += GAMMA[(p2[j+0] & 0xff0000) >> 16] << 0;
	r += GAMMA[(p2[j+1] & 0xff0000) >> 16] << 1;
	r += GAMMA[(p2[j+2] & 0xff0000) >> 16] << 0;
	r  = GAM2RGB[r >> 4] << 16;
	g  = GAMMA[(p0[j+0] & 0xff00) >> 8] << 0;
	g += GAMMA[(p0[j+1] & 0xff00) >> 8] << 1;
	g += GAMMA[(p0[j+2] & 0xff00) >> 8] << 0;
	g += GAMMA[(p1[j+0] & 0xff00) >> 8] << 1;
	g += GAMMA[(p1[j+1] & 0xff00) >> 8] << 2;
	g += GAMMA[(p1[j+2] & 0xff00) >> 8] << 1;
	g += GAMMA[(p2[j+0] & 0xff00) >> 8] << 0;
	g += GAMMA[(p2[j+1] & 0xff00) >> 8] << 1;
	g += GAMMA[(p2[j+2] & 0xff00) >> 8] << 0;
	g  = GAM2RGB[g >> 4] << 8;
	b  = GAMMA[(p0[j+0] & 0xff) >> 0] << 0;
	b += GAMMA[(p0[j+1] & 0xff) >> 0] << 1;
	b += GAMMA[(p0[j+2] & 0xff) >> 0] << 0;
	b += GAMMA[(p1[j+0] & 0xff) >> 0] << 1;
	b += GAMMA[(p1[j+1] & 0xff) >> 0] << 2;
	b += GAMMA[(p1[j+2] & 0xff) >> 0] << 1;
	b += GAMMA[(p2[j+0] & 0xff) >> 0] << 0;
	b += GAMMA[(p2[j+1] & 0xff) >> 0] << 1;
	b += GAMMA[(p2[j+2] & 0xff) >> 0] << 0;
	b  = GAM2RGB[b >> 4] << 0;
	*p++ = a | r | g | b;
#endif
      }
